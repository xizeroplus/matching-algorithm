...
void Motree::insertToBucket(const vector<int> &mark, int bucketID, int layer, int subID)
{
    ...
    if(!layer) //the first layer needs to *10 because first two layers have 10 cells
    ...
    if (layer == mLevel - 1)
    {
        for (int i = 0; i < mark[layer].size(); i++)
        {
            ...
            for (int k = 0; k < pubdata[id].size(); k++)
            {
                ...
                if it is, simply delete pubdata[id][k], just like what "deleteFromPubdata" function does (line 472)
