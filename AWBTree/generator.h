...
public:
    ...
    generator(int subs, int pubs, int atts, int cons, int m, int attDis, int valDis, int valDom, double alpha) : subs(subs), pubs(pubs), atts(atts), cons(cons), m(m), attDis(attDis), valDis(valDis), valDom(valDom),
        alpha(alpha)
    {
    }
...
class intervalGenerator
{
    ...
    void GenUniformValues(IntervalSub& sub, int valDom, double w);
    //void GenUnMatchValues(IntervalSub& sub, int valDom);


    void GenUniformValues(Pub& pub, int valDom);

    void GenZipValues(IntervalSub& sub, int valDom, double alpha);
    void GenZipValues(IntervalSub& sub, int valDom, double alpha, double w);

    void GenZipValues(Pub& pub, int valDom, double alpha);
    ...
    IntervalSub GenUnMatchSub(int id, int size, int atts, int attDis, int valDis, int valDom, double alpha, double width);
